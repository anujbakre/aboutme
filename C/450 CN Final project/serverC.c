...
{    
    ...
    if((status = getaddrinfo("localhost", portC_UDP, &info, &servinfo)) != 0) 		// From Beej guide
    {
    ...
    {
        if ((socket_fd_C = socket(p->ai_family, p->ai_socktype,p->ai_protocol)) == -1) // create socket
        {
            perror("backend server C UDP listener: socket");
            continue;
        }

        if (bind(socket_fd_C, p->ai_addr, p->ai_addrlen) == -1) 				// bind the socket
        {
            close(socket_fd_C);
            perror("backend server C UDP listener: bind");
            continue;
        }
    ...
    {
        fprintf(stderr, "backend server C UDP listener: failed to bind socket\n");
        return 2;
    }
    ...
    l = sizeof(s);
    if (getsockname(socket_fd_C, (struct sockaddr *)&s, &l) == -1)
        perror("getsockname");
    else
    ...
    if ((n = recvfrom(socket_fd_C, rec_buff, MAX_LENGTH -1 , 0,(struct sockaddr *)&their_addr, &addr_len)) == -1)			// recieve opeartion and # of integers from aws
    {
        perror("backend server C : recvfrom");
        exit(1);
    }
    ...
    {
          memset(rec_buff_new,0,sizeof(rec_buff));
         if ((n = recvfrom(socket_fd_C, rec_buff_new, 3*MAX_LENGTH , 0,(struct sockaddr *)&their_addr, &addr_len)) < 0 )
        {
            perror("backend server C : recvfrom");
            exit(1);
        }
    ...
    memset(tmp_buff_C, 0, sizeof(tmp_buff_C));
    memset(send_data, 0, sizeof(send_data)); 
    
    sprintf(tmp_buff_C, "%ld" , send_result);

    send_data[0] = 'C';
    send_data[1] = '\t';
    strcat(send_data,tmp_buff_C);

   
   
   sendUDP_backserver(portAWS_UDP, (struct sockaddr *)&their_addr, send_data, socket_fd_C);				// send result to aws`
