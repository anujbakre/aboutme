...
void data_parse(long *data_array, long *buff)			// store recived data in aarray
{
    int i = 0;
...
int main()
{
    ...
    size_t addr_len;
    int n;
    char rec_buff[MAX_LENGTH], tmp_buff[MAX_LENGTH];
    int operation;
    long min, max;
    long send_result = 0;
    char *reduction;
    char send_data[3*MAX_LENGTH];
    char tmp_buff_A[3*MAX_LENGTH];
while(1)
{   
    rec_no = 0;
     f_len =0;
    ...
    if((status = getaddrinfo("localhost", portA_UDP, &info, &servinfo)) != 0) 	// From Beej guide
    {
    ...
    {
        if ((socket_fd_A = socket(p->ai_family, p->ai_socktype,p->ai_protocol)) == -1) 		// create socket
        {
            perror("backend server A UDP listener: socket");
            continue;
        }

        if (bind(socket_fd_A, p->ai_addr, p->ai_addrlen) == -1) 			// bind the socket
        {
            close(socket_fd_A);
            perror("backend server A UDP listener: bind");
            continue;
        }
    ...
    {
        fprintf(stderr, "backend server A UDP listener: failed to bind socket\n");
        return 2;
    }
    ...
    l = sizeof(s);
    if (getsockname(socket_fd_A, (struct sockaddr *)&s, &l) == -1)
        perror("getsockname");
    ...
    if ((n = recvfrom(socket_fd_A, rec_buff, MAX_LENGTH -1 , 0,(struct sockaddr *)&their_addr, &addr_len)) == -1)			// recieve opeartion and # of integers from aws
    {
        perror("backend server A : recvfrom");
        exit(1);
    }
    ...
    {
         memset(rec_buff_new,0,sizeof(rec_buff));
         if ((n = recvfrom(socket_fd_A, rec_buff_new, 3* MAX_LENGTH , 0,(struct sockaddr *)&their_addr, &addr_len)) < 0 )
        {
            perror("backend server A : recvfrom");
            exit(1);
        }
        ...
        //parse bufer recived to read data and stoe it in array. 
    ...
    switch(operation)				// perfrom operation on data recieved
    {
        ...
        default: fprintf(stderr,"error recieving data from backend servers: invalid operation");
                    exit(1);
                    break;
    ...
    memset(tmp_buff_A, 0, sizeof(tmp_buff_A)); 
    memset(send_data, 0, sizeof(send_data));
    
    sprintf(tmp_buff_A, "%ld" , send_result);

    send_data[0] = 'A';
    send_data[1] = '\t';
    strcat(send_data,tmp_buff_A);

   
   sendUDP_backserver(portAWS_UDP, (struct sockaddr *)&their_addr, send_data, socket_fd_A);			// send result to aws`

    printf("The Server A has successfully finished sending the reduction value to AWS server\n");
  shutdown(socket_fd_A, 2);
